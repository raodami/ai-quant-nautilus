...
class GeneticAlgorithm:
    ...
    def _crossover(self, parents: list) -> list:
        ...
        for i in range(0, len(parents) - 1, 2):
            ...
            if random.random() < self.crossover_rate:
                child1, child2 = parent1.crossover(parent2, self.crossover_method)
                # Preserve parameter values
                child1.parameter_values = self._merge_parameters(parent1, parent2, child1)
                child2.parameter_values = self._merge_parameters(parent2, parent1, child2)
                offspring.extend([child1, child2])
            else:
                offspring.extend([parent1.clone(), parent2.clone()])
        return offspring

    def _mutation(self, offspring: list) -> list:
    ...
    def _merge_parameters(self, parent1: Individual, parent2: Individual, child: Individual) -> dict:
        """Merge parameter values from parents to child."""
        merged = {}
        for param in self.parameters:
            if random.random() < 0.5:
                merged[param.name] = parent1.parameter_values.get(param.name, param.default)
            else:
                merged[param.name] = parent2.parameter_values.get(param.name, param.default)
        return merged
...
def _crossover(
    ...
    if method == CrossoverMethod.SINGLE_POINT:
        ...
        points = sorted(random.sample(range(length), 2))
